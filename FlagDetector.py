...
class FlagDetector:
    ...
    def morphological(self, image):
        ...
        image = cv.morphologyEx(image, cv.MORPH_CLOSE, kernel)  # Close small gaps
        image = cv.morphologyEx(image, cv.MORPH_OPEN, kernel = cv.getStructuringElement(cv.MORPH_ELLIPSE, (1, 1)))   # Remove small noise
        
        cv.imshow('frame_masked', cv.resize(image, (self.display_width, self.display_height)))
    ...
    def extract_and_ocr(self, frame, corners):
        ...
        results = self.reader.readtext(warped, allowlist='0123456789', detail=1)
